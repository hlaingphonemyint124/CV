...
(function(){
  ...
  function drawConns(){
    for(var i=0;i<P.length;i++)for(var j=i+1;j<P.length;j++){
      ...
      if(d<110){ctx.beginPath();ctx.strokeStyle='rgba(232,0,13,'+((1-d/110)*0.07)+')';ctx.lineWidth=0.4;ctx.moveTo(P[i].x,P[i].y);ctx.lineTo(P[j].x,P[j].y);ctx.stroke();}
    }
  }
  function frame(){
    ...
    S.forEach(function(s){
      ...
      var px=CX+(s.x-CX)*(1000/s.z),py=CY+(s.y-CY)*(1000/s.z);
      if(px<0||px>cv.width||py<0||py>cv.height)return;
      ctx.beginPath();ctx.arc(px,py,Math.max(.2,s.sz*(1000/s.z)*0.5),0,Math.PI*2);
      ctx.fillStyle=s.red?'rgba(232,0,13,'+((0.5+Math.sin(s.tw)*0.5)*.8)+')':'rgba(255,255,255,'+((0.5+Math.sin(s.tw)*0.5)*.9)+')';
      ctx.fill();
    });
    P.forEach(function(p){
      ...
      ctx.fillStyle=p.red?'rgba(232,0,13,'+a+')':'rgba(255,255,255,'+a+')';ctx.fill();
    });
    drawConns();
...
(function(){
  ...
  for(var i=0;i<nodes.length;i++){
    var y=1-(i/(nodes.length-1))*2,r=Math.sqrt(1-y*y),t=phi*i;
  ...
  function rot(x,y,z,arx,ary){
    var nx=x*Math.cos(ary)+z*Math.sin(ary),nz=-x*Math.sin(ary)+z*Math.cos(ary);
    var ny=y*Math.cos(arx)-nz*Math.sin(arx),nz2=y*Math.sin(arx)+nz*Math.cos(arx);
  ...
  function anim(){
    ...
    for(var lat=-80;lat<=80;lat+=40){
      ...
      for(var lon=0;lon<=360;lon+=5){
        var lonr=lon*Math.PI/180,p=rot(cr*Math.cos(lonr),y0,cr*Math.sin(lonr),rx,ry),sc=(p.z+R*2)/(R*3);
    ...
    for(var lon=0;lon<360;lon+=40){
      var lonr=lon*Math.PI/180;ctx.beginPath();
      for(var lat=-90;lat<=90;lat+=5){
        var lr=lat*Math.PI/180,y0=R*Math.sin(lr),cr=R*Math.cos(lr);
        var p=rot(cr*Math.cos(lonr),y0,cr*Math.sin(lonr),rx,ry),sc=(p.z+R*2)/(R*3);
    ...
    nodes.slice().sort(function(a,b){return a.rz-b.rz;}).forEach(function(n){
      var dep=(n.rz+R)/(2*R),front=n.rz>0,sc=0.6+dep*0.6,sx=CX+n.rx*sc,sy=CY+n.ry*sc;
      if(!front){
        ctx.beginPath();ctx.arc(sx,sy,3*sc,0,Math.PI*2);ctx.fillStyle=n.c+'40';ctx.fill();
        ctx.fillStyle='rgba(255,255,255,'+(0.1+dep*0.15)+')';
        ctx.font=Math.floor(9*sc)+'px Space Mono,monospace';ctx.textAlign='center';ctx.fillText(n.l,sx,sy+3);
      } else {
